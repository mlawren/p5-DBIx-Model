...
#<<<
  ...
  {$_}}'name','nullable','table','type';if (@ATTRS_UNEX){map {local
  ...
  $_[0]}$_[0]{'chain'}// undef}sub name {$_[0]->__ro($_[1])if @_ > 1;$_[0]{
  'name'}}sub nullable {$_[0]->__ro($_[1])if @_ > 1;$_[0]{'nullable'}}sub
  primary {$_[0]->__ro($_[1])if @_ > 1;$_[0]{'primary'}//= eval {$INLINE->{
  'primary'}->{'isa'}->($INLINE->{'primary'}->{'default'}->($_[0]))};
  Carp::croak('invalid (DBIx::Model::Column::primary) default value: ' .$@)if
  $@;$_[0]{'primary'}}sub ref_count {if (@_ > 1){$_[0]{'ref_count'}=eval {
  $INLINE->{'ref_count'}->{'isa'}->($_[1])};Carp::croak(
